...
- [x] **Rep detection**: find local extrema in smoothed y — zero-crossings of velocity that exceed an amplitude threshold to ignore wobbles. Output: `{startFrame, endFrame}[]`.
...
- [ ] **PCA to determine axis of movement**: we default to vertical movement for reps, but you could look at other planes. Manual specification is possible, but automatic detection with PCA feels nicer. Tricky thing: unrack and rerack.
